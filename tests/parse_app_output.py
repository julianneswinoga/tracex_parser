...
class AppEvent(NamedTuple):
    ...
    raw_event: Optional[int]
    raw_priority: Optional[int]
...
def parse_events_txt(lines: List[str]) -> List[AppEvent]:
    ...
    for event_str in lines:
        ...
        print(event_str_split)  # Just for pytest debug
        ...
        if event_str_split[4].lower() == 'interrupt':
            ...
            priority_base = 10
        # No idea what the "TX RUNNING" (6) event is doing here . It's not
        # at all in the file, so it must be generated by other means? `--------`
        raw_event = None if '-' in event_str_split[13] else int(event_str_split[13], 16)
        raw_priority = None if '-' in event_str_split[14] else int(event_str_split[14], 16)
        ...
        app_event = AppEvent(
            ...
            priority=int(event_str_split[5].split(maxsplit=1)[0], priority_base),  # Ignore the '?'
